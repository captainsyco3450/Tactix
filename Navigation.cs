...
void buildGraph()
    ...
    void findPath(int startNumber, int targetNumber)
    {
        ...
        //}
        //if (nav.neighbors.Contains(targetNode.transform.gameObject))
        // {
        //Debug.Log("Found at " + node.name);

        //foreach (GameObject neighbor in nav.neighbors)
        //{
        //    dist2 = Vector3.Distance(targetNode.transform.position, neighbor.transform.position);
        //    distList.Add(dist2);

        //}
        //foreach (GameObject neighbor in nav.neighbors)
        //{
        //    if (Vector3.Distance(targetNode.transform.position, neighbor.transform.position) == distList.Min())
        //    {
        //    path.Add(neighbor.transform);
        //    neighbor.GetComponent<Renderer>().material.color = new Color(143, 26, 0, 1f);
        ...
        if (graphBuilt)
        {
            ...
            if (!needPath)
            {
                ...
                if (path.Contains(GameObject.Find("NavNode (" + targetNum + ")").transform))
                {
                    ...
                    if (path[count -1].GetComponent<NavNode>().id == targetNum)
                    {
                        // shouldWalk = false;
    ...
    void LookAtNode(Transform node)
    {
        ...
        this.controller.transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction), 5 * Time.deltaTime);
